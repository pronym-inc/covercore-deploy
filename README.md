This is a set of Ansible scripts for deploy the covercore Django application.
...
1. Run `vagrant up`
1. There is a final step where we open up [http://covercorevagrant.local](http://genesishealth.local) in Chrome, and that will surely fail on non-OSX Linux machines, but only after the rest of the command has finished.